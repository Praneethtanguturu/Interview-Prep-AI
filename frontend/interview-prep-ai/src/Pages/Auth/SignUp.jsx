...
const SignUp = ({setCurrentPage}) => {
  ...
  const handleSignUp = async(e) => {
    ...
    if(!fullName) { setError("Please enter an full name."); return;}
    if(!validateEmail(email)) { setError("Please enter a valid email address."); return; }
    if(!password) { setError("Please enter an password"); return; }
    setError("");
  ...
  return (
    <div className="">
      ...
      <form onSubmit={handleSignUp}>
        ...
        <p className="text-[13px] text-slate-800 mt-3"> Already an Account?{" "} <button className="font-medium text-primary underline cursor-pointer" onClick={()=>{setCurrentPage("login");}}> Login </button> </p>
      </form>
